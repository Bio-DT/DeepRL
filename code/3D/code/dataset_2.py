...
from layers import SoftOneHot, SoftOneHot_1, SoftOneHot_2
from utils import ATOM_TYPES


class DataProcessor_2(object):
    def __init__(self, args, mode="train"):
        ...
        self.distance_expand1 = SoftOneHot_1(*args.dist_one_hot_param1, gamma=args.gamma1)
        self.distance_expand2 = SoftOneHot_1(*args.dist_one_hot_param2, gamma=args.gamma2, normalize=True)
        #self.distance_expand3 = SoftOneHot_1(*args.dist_one_hot_param2, gamma=args.gamma2, normalize=True)
    ...
    def get_input_from_data(self, data):
        ...
        # print("====l_Z=====",l_Z.sum())
        # print("====l_R=====",l_R.sum())
        # print("====torch.Tensor(ligand_adj)=====",torch.Tensor(ligand_adj).sum())
...
class MoleculeDataset_2(Dataset):
    def __init__(
        ...
        self.data_dir_2 = args.data_dir_2
        self.key_dir_2 = args.key_dir_2
...
class PDBbindDataset_2(MoleculeDataset_2):
    def __init__(self, args, mode="train" ):
        super().__init__(args, mode)

        self.processor = DataProcessor_2(args, mode=self.mode) ########################################

        if not os.path.exists(self.key_dir_2 + f"{mode}_keys.pkl"):
            print(f"No {mode}_keys.pkl exists!")
            exit()
        with open(self.key_dir_2 + f"{mode}_keys.pkl", "rb") as f:
            self.key_list_2 = pickle.load(f)

    def __len__(self):
        return len(self.key_list_2)

    def __getitem__(self, idx):
        key = self.key_list_2[idx]
        # print("======key=====",key)
        # exit()
        try:
            with open(self.data_dir_2 + key, "rb") as f:
                data = pickle.load(f)
