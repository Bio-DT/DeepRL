...
class Generator(DataProcessor):
    def __init__(self, args, model, device):
        ...
        for k in self.keys:
            with open(args.data_dir + "/" + k, "rb") as f:
                self.data_list.append(pickle.load(f))   #.../DeepICL-master/example
    ...
    def _generate_molecule(
        ...
        # print("=====o_R========",self.o_R)
        # exit()
        l_Z_init = self.o_Z  # torch.zeros(1, args.num_ligand_atom_feature)  # COM
        l_R_init = self.o_R  # self.o_R = torch.zeros(1, self.num_dim)
        ...
        while i < self.max_num_atom:
            ...
            # Add token
            l_Z_in = torch.cat([self.f_Z, l_Z_init], 0) #self.f_Z = torch.zeros(1, args.num_ligand_atom_feature)  # focus
