...
def get_data(args):
    ...
    data['drugfeature'] = pd.read_csv(args.data_dir + 'Drug_mol2vec.csv', header=None).iloc[:, 1:].to_numpy()  ###drug_feature
    data['diseasefeature'] = pd.read_csv(args.data_dir + 'DiseaseFeature.csv', header=None).iloc[:, 1:].to_numpy()
    data['proteinfeature'] = pd.read_csv(args.data_dir + 'Protein_ESM.csv', header=None).iloc[:, 1:].to_numpy()  ###protein_feature
...
def data_part(data, args):
    k = args.iteration
    skf = StratifiedKFold(n_splits=k, random_state=None, shuffle=False)  #False
    X = data['all_drdi']
    Y = data['all_label']
