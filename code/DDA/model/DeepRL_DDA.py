...
from DeepRL_DDA import Transformer_lar, ModelArgs

device = torch.device('cuda')


class DeepRL_DDA_model(nn.Module):
    def __init__(self, args):
        super(DeepRL_DDA_model, self).__init__()
        self.args = args
        ...
        self.mlp = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Dropout(0.5), #0.5-F
            nn.Linear(1024, 1024),
            nn.ReLU(),
            nn.Dropout(0.5), #0.5-F
            nn.Linear(1024, 256),
            nn.ReLU(),
            nn.Dropout(0.45),  #0.45-F
            nn.Linear(256, 2)
        )
    ...
    def forward(self, drdr_graph, didi_graph, drdipr_graph, drug_feature, disease_feature, protein_feature, sample):
        ...
        dr_lar = self.llama(dr).permute(1, 0, 2)
        di_lar = self.llama(di).permute(1, 0, 2)

        dr = self.drug_trans(dr)
        di = self.disease_trans(di)

        alpha = 0.4  #0.4-F 0.2-B  0.6-C
        dr = alpha*dr + (1-alpha)*dr_lar
        di = alpha*di + (1-alpha)*di_lar
