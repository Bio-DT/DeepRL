...
save_model = './DeepRL/DTA/save_model'
save_result = './DeepRL/DTA/save_result'
...
for dataset in datasets:
    ...
    if ((not os.path.isfile(processed_data_file_train)) or (not os.path.isfile(processed_data_file_test))):
        ...
        train_data = TestbedDataset(root='./DeepRL/DTA/data', dataset=dataset+'_train')
        test_data = TestbedDataset(root='./DeepRL/DTA/data', dataset=dataset+'_test')
       ...
       # ##load model
        ...
        for epoch in range(NUM_EPOCHS):
            ...
            if ret[-1]>best_ci:  ##ret[1]<best_mse:  ret[-1]>best_ci
                torch.save(model.state_dict(), name)   ###save model
